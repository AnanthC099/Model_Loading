...
// 13-SwapchainImagesAndViews --  For Color Images
...
VetextData vetextData_position;
VetextData vetextData_normal;
VetextData vetextData_texcoord;
VetextData vetextData_index;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindowEx(
		WS_EX_APPWINDOW,
		szAppName,
		TEXT("Vulkan On Windows |  Tea Pot - 12th November 2025 Header File based model loading - Texture_White_Light"),
...
VkResult initialise(void)
{
	...
	// Position
	 ...
	 // Declare Temparory Arrays to hold triangle vertices
	 float vert[3][3];
	 float norm[3][3];
	 float tex[3][2];
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "initialize() -> createTexture() for stone is failed %d\n", vkResult);
		fflush(gpFile);
		return vkResult;
	}
	else
	{
		fprintf(gpFile, "initialize() -> createTexture() for stone Succeeded\n");
		fflush(gpFile);
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "initialise() -> createIndexBuffer() is failed. %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "initialise() -> createIndexBuffer() is successed. \n");
	}
	...
	// setDefaultClearDepth value ,this value is float
	memset((void*)&vkClearDepthStencilValue, 0, sizeof(VkClearDepthStencilValue));
	vkClearDepthStencilValue.depth = 1.0f;
	// setDefault Clear stencil value, this value is uint32_t 
	vkClearDepthStencilValue.stencil = 0.0;
...
VkResult resize(int width, int height)
{
	...
	if (vkImageView_depth)
	{
		...
		//fprintf(gpFile, "resize() -> vkDestroyImageView() for vkImageView_depth is done. \n");
	}
	...
	if (vkDeviceMemory_depth)
	{
		...
		//fprintf(gpFile, "resize() -> vkFreeMemory() for vkDeviceMemory_depth is done. \n");
	}
	// Destroy depth image
	if (vkImage_depth)
	{
		...
		//fprintf(gpFile, "resize() -> vkDestroyImage() for vkImage_depth is done. \n");
	}
...
void uninitialise(void)
{
	...
	if (uniformData.vkBuffer)
	{
		...
		fprintf(gpFile, "uninitialise() -> vkDestroyBuffer() is done for uniformData.vkBuffer\n");
	}
	...
	if (uniformData.vkDeviceMemory)
	{
		...
		fprintf(gpFile, "uninitialise() -> vkFreeMemory() is done for uniformData.vkDeviceMemory\n");
	}
	...
	if (vetextData_index.vkDeviceMemory)
	{
		vkFreeMemory(vkDevice, vetextData_index.vkDeviceMemory, NULL);
		vetextData_index.vkDeviceMemory = VK_NULL_HANDLE;
	}

	if (vetextData_index.vkBuffer)
	{
		vkDestroyBuffer(vkDevice, vetextData_index.vkBuffer, NULL);
		vetextData_index.vkBuffer = VK_NULL_HANDLE;
	}

	if (vetextData_texcoord.vkDeviceMemory)
	{
		vkFreeMemory(vkDevice, vetextData_texcoord.vkDeviceMemory, NULL);
		vetextData_texcoord.vkDeviceMemory = VK_NULL_HANDLE;
	}

	if (vetextData_texcoord.vkBuffer)
	{
		vkDestroyBuffer(vkDevice, vetextData_texcoord.vkBuffer, NULL);
		vetextData_texcoord.vkBuffer = VK_NULL_HANDLE;
	}

	if (vetextData_normal.vkDeviceMemory)
	{
		vkFreeMemory(vkDevice, vetextData_normal.vkDeviceMemory, NULL);
		vetextData_normal.vkDeviceMemory = VK_NULL_HANDLE;
	}

	if (vetextData_normal.vkBuffer)
	{
		vkDestroyBuffer(vkDevice, vetextData_normal.vkBuffer, NULL);
		vetextData_normal.vkBuffer = VK_NULL_HANDLE;
	}

	if (vetextData_position.vkDeviceMemory)
	{
		vkFreeMemory(vkDevice, vetextData_position.vkDeviceMemory, NULL);
		vetextData_position.vkDeviceMemory = VK_NULL_HANDLE;
	}

	if (vetextData_position.vkBuffer)
	{
		vkDestroyBuffer(vkDevice, vetextData_position.vkBuffer, NULL);
		vetextData_position.vkBuffer = VK_NULL_HANDLE;
	}
	...
	// vkImageView_depth destroy
	if (vkImageView_depth)
	{
		vkDestroyImageView(vkDevice, vkImageView_depth, NULL);

		vkImageView_depth = VK_NULL_HANDLE;
		fprintf(gpFile, "uninitialise() -> vkDestroyImageView() for vkImageView_depth is done. \n");
	}
	// ---------------------
	// Depth related
...
VkResult createVulkanInstance(void)
{
	...
	vkApplicationInfo.apiVersion		 = VK_API_VERSION_1_4; // version update kela vulkan ch ki hita 1_4 change karne
...
VkResult createImagesAndImageView()
{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createImagesAndImageView() -> getSupportedDepthFormat() is failed due to %d error \n", vkResult);
		return (vkResult);
	}
	else
	{
	...
	// Hech structure VkImageCreateInfo aplya la texture la lagnar ahe
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createImagesAndImageView() -> vkCreateImage() is failed  due to %d error \n",vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createImagesAndImageView() -> vkCreateImage() is successed \n");
	}
	// ==================================== Memory Requirements For Depth Image =============================================;
	VkMemoryRequirements vkMemoryRequirements;
	memset((void*)&vkMemoryRequirements, 0, sizeof(VkMemoryRequirements));

	// Memory Requirments for depth images
	...
	*/
	...
	{
		fprintf(gpFile, "createUniformBuffer() -> vkBindBufferMemory() \n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createImagesAndImageView() -> vkCreateImageView() for vkImageView_depth is failed : due to %d error \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createImagesAndImageView() -> vkGetSwapchainImagesKHR() for vkImageView_depth is successed : \n");
	}
...
VkResult getSupportedDepthFormat(void)
{
	...
	// Code
	fprintf(gpFile, "\n=======================================================================================\n");
	fprintf(gpFile, "getSupportedDepthFormat() : 30th July 2025\n\n");
...
VkResult createVertexBuffer(void)
{
	...
	memset((void *)&vetextData_position, 0, sizeof(VetextData));
	...
	*/
	...
	vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vetextData_position.vkBuffer);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkCreateBuffer() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkCreateBuffer() \n");
	}
	...
	vkGetBufferMemoryRequirements(vkDevice, vetextData_position.vkBuffer, &vkMemoryRequirements);
	...
	*/
	...
	vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vetextData_position.vkDeviceMemory);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkAllocateMemory() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkAllocateMemory() \n");
	}
	...
	vkResult = vkBindBufferMemory(vkDevice, vetextData_position.vkBuffer, vetextData_position.vkDeviceMemory, 0);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkBindBufferMemory() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkBindBufferMemory() \n");
	}
	...
	vkResult = vkMapMemory(vkDevice, vetextData_position.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkMapMemory() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkMapMemory() \n");
	}
	...
	vkUnmapMemory(vkDevice, vetextData_position.vkDeviceMemory);
    //------------------------------------------------ NORMAL -----------------------------------------------------------
	// Sphere Normal
	//Step 04: memset our global vertexData_Position. 
	memset((void*)&vetextData_normal, 0, sizeof(VetextData));
	...
	*/
	...
	vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vetextData_normal.vkBuffer);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkCreateBuffer() vetextData_normal.vkBuffer is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkCreateBuffer() vetextData_normal.vkBuffer \n");
	}
	...
	vkGetBufferMemoryRequirements(vkDevice, vetextData_normal.vkBuffer, &vkMemoryRequirements);
	...
	*/
	...
	vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vetextData_normal.vkDeviceMemory);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkAllocateMemory() vetextData_normal.vkDeviceMemory is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkAllocateMemory() vetextData_normal.vkDeviceMemory \n");
	}
	...
	vkResult = vkBindBufferMemory(vkDevice, vetextData_normal.vkBuffer, vetextData_normal.vkDeviceMemory, 0);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkBindBufferMemory() vetextData_normal is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkBindBufferMemory() vetextData_normal \n");
	}
	...
	vkResult = vkMapMemory(vkDevice, vetextData_normal.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkMapMemory() vetextData_normal is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkMapMemory() vetextData_normal \n");
	}
	...
	vkUnmapMemory(vkDevice, vetextData_normal.vkDeviceMemory);

	//------------------------------------------------ TextCoord -----------------------------------------------------------
	// Sphere TexCoord
	//Step 04: memset our global vertexData_Position. 
	memset((void*)&vetextData_texcoord, 0, sizeof(VetextData));
	...
	*/
	...
	vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vetextData_texcoord.vkBuffer);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkCreateBuffer() vetextData_texcoord.vkBuffer is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkCreateBuffer() vetextData_texcoord.vkBuffer \n");
	}
	...
	vkGetBufferMemoryRequirements(vkDevice, vetextData_texcoord.vkBuffer, &vkMemoryRequirements);
	...
	*/
	...
	vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vetextData_texcoord.vkDeviceMemory);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkAllocateMemory() vetextData_texcoord.vkDeviceMemory is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkAllocateMemory() vetextData_texcoord.vkDeviceMemory \n");
	}
	...
	vkResult = vkBindBufferMemory(vkDevice, vetextData_texcoord.vkBuffer, vetextData_texcoord.vkDeviceMemory, 0);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkBindBufferMemory() vetextData_texcoord is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkBindBufferMemory() vetextData_texcoord \n");
	}
	...
	vkResult = vkMapMemory(vkDevice, vetextData_texcoord.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createVertexBuffer() -> vkMapMemory() vetextData_texcoord is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createVertexBuffer() -> vkMapMemory() vetextData_texcoord \n");
	}
	...
	vkUnmapMemory(vkDevice, vetextData_texcoord.vkDeviceMemory);
...
VkResult createIndexBuffer(void)
{
	...
	memset((void*)&vetextData_index, 0, sizeof(VetextData));
	...
	*/
	...
	vkResult = vkCreateBuffer(vkDevice, &vkBufferCreateInfo, NULL, &vetextData_index.vkBuffer);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createIndexBuffer() -> vkCreateBuffer() for vetextData_index is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createIndexBuffer() -> vkCreateBuffer() for vetextData_index\n");
	}
	...
	vkGetBufferMemoryRequirements(vkDevice, vetextData_index.vkBuffer, &vkMemoryRequirements);
	...
	*/
	...
	vkResult = vkAllocateMemory(vkDevice, &vkMemoryAllocateInfo, NULL, &vetextData_index.vkDeviceMemory);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createIndexBuffer() -> vkAllocateMemory() vetextData_index.vkDeviceMemory is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createIndexBuffer() -> vkAllocateMemory() for vetextData_index.vkDeviceMemory \n");
	}
	...
	vkResult = vkBindBufferMemory(vkDevice, vetextData_index.vkBuffer, vetextData_index.vkDeviceMemory, 0);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createIndexBuffer() -> vkBindBufferMemory() vetextData_index is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createIndexBuffer() -> vkBindBufferMemory() vetextData_index \n");
	}
	...
	vkResult = vkMapMemory(vkDevice, vetextData_index.vkDeviceMemory, 0, vkMemoryAllocateInfo.allocationSize, 0, &data);
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createIndexBuffer() -> vkMapMemory() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createIndexBuffer() -> vkMapMemory() \n");
	}
	...
	vkUnmapMemory(vkDevice, vetextData_index.vkDeviceMemory);
...
VkResult createUniformBuffer(void) 
{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createUniformBuffer() -> vkCreateBuffer() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createUniformBuffer() -> vkCreateBuffer() \n");
	}
	...
	*/
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createUniformBuffer() -> vkAllocateMemory() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
		fprintf(gpFile, "createUniformBuffer() -> vkAllocateMemory() \n");
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createUniformBuffer() -> vkBindBufferMemory() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
	else
	{
...
VkResult updateUniformBuffer(void)
{
	...
	translateMatrix = glm::translate(glm::mat4(1.0), glm::vec3(0.0f, 0.0f, -2.0f));
	glm::mat4 rotationMatrix = glm::rotate(glm::mat4(1.0f), glm::radians(angleTeapot), glm::vec3(0.0f, 1.0f, 0.0f));

	myUniformData.modelMatrix      = translateMatrix * rotationMatrix;
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "updateUniformBuffer() -> vkMapMemory() is failed due to error : %d \n", vkResult);
		return (vkResult);
	}
...
VkResult createDescriptorSet(void)
{
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gpFile, "createDescriptorSet() -> vkAllocateDescriptorSets() is failed %d \n", vkResult);
		return (vkResult);
	}
	else
	{
...
VkResult createRenderPass(void)
{
	...
	// Till Now array size is One but now we are considering depth so it is Two
	...
	vkAttachmentReference_depth.attachment = 1; // 1 means oth index no. in above array. i.e depth attachment
	...
	// This Count Should be array count which belongs to attachment reference not belongs to attachment description
	// This Count Should be the count of vkAttachmentReference used for colors
	// aata Paryat Error nahi aali karan color attachment 1 ch ahe and attachment description count 1 ch ahe pan aaj total attachment count asnar ahe 2
	// ani color attachment count 1 ch ahe tya mule aaj te chuku shakte --> this is recommended change
...
VkResult createFramebuffers(void)
{
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		//Step 01: declare an array of VkImageView equal to the number of attachemnt means in our example array of 1 memeber
		// Now Array size is 2 for color and depth both
...
VkResult buildCommandBuffer(void) 
{
	...
	for (uint32_t i = 0; i < swapchainImageCount; i++)
	{
		...
		vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 0, 1, &vetextData_position.vkBuffer, vkDeviceSize_offset_position_array);


		// Textcoord
		VkDeviceSize vkDeviceSize_offset_textcoord_array[1];
		memset((void*)vkDeviceSize_offset_textcoord_array, 0, sizeof(VkDeviceSize) * _ARRAYSIZE(vkDeviceSize_offset_textcoord_array));
		vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 1, 1, &vetextData_texcoord.vkBuffer, vkDeviceSize_offset_textcoord_array);
		...
		vkCmdBindVertexBuffers(vkCommandBuffer_array[i], 2, 1, &vetextData_normal.vkBuffer, vkDeviceSize_offset_normal_array);

		

		// unsigned short sphere_elements  unsigned short ahe so VK_INDEX_TYPE_UINT32 should be VK_INDEX_TYPE_UINT16
		// unsigned int should be VK_INDEX_TYPE_UINT32
		vkCmdBindIndexBuffer(vkCommandBuffer_array[i], vetextData_index.vkBuffer, 0, VK_INDEX_TYPE_UINT32);
...
{
	// code 
	fprintf(gpFile, "\nRMK_VALIDATION : debugReportCallBack() -> %s (%d) = %s \n", pLayerPrefix, messageCode, pMessage);
